...
{
    const s = addSection("Æfing 4: While loop", "Byrja með breytu með gildi 1 og tvöfalda hana þangað til hún verður orðin stærri en 100")
    let tala = 1
    while (tala < 100) {
        const p = document.createElement("p")
        ...
        s.appendChild(p)
        tala = tala * 2
        // líka hægt að skrifa tala *= 2 sem gerir það sama og tala = tala * 2
...
{
    const s = addSection("Æfing 5: While loop", "Prenta allar oddatölur undir 20")
    let oddatala = 1;
    while (oddatala < 20) {
        const p = document.createElement("p")
...

{
    ...
    do {
        ...
        const p = document.createElement("p")
        p.textContent = `yayy þú rúllaðir ${roll}`
        s.appendChild(p)
}
...

{
    ...
    const data = { a: 10, b: 'Kinga', c: true, d: 3}
